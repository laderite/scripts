...
function EspObject:Construct()
	...
	self.drawings = {
		...
		visible = {
            ...
            lifeforceBarOutline = self:_create("Line", { Thickness = 3, Visible = false }),
...
function EspObject:Update()
    ...
    self.lifeforce, self.maxLifeforce = interface.getLifeforce(self.player);
...
function EspObject:Render()
    ...
    if visible.level.Visible then
        local level = visible.level;
        level.Text = string.format("LVL %d (%d)", self.level or 0, self.trueLevel or 0);
        level.Size = interface.sharedSettings.textSize;
        level.Font = interface.sharedSettings.textFont;
        level.Color = parseColor(self, options.levelColor[1]);
        level.Transparency = options.levelColor[2];
        level.Outline = options.levelOutline;
        level.OutlineColor = parseColor(self, options.levelOutlineColor, true);
        level.Position = (corners.topLeft + corners.topRight)*0.5 - Vector2.yAxis*level.TextBounds.Y - LEVEL_OFFSET;
    end
    ...
    visible.lifeforceBar.Visible = enabled and onScreen and options.lifeforceBar;
    visible.lifeforceBarOutline.Visible = visible.lifeforceBar.Visible and options.lifeforceBarOutline;
    if visible.lifeforceBar.Visible then
        local barFrom = corners.topRight + LIFEFORCE_BAR_OFFSET;
        local barTo = corners.bottomRight + LIFEFORCE_BAR_OFFSET;

        local lifeforceBar = visible.lifeforceBar;
        lifeforceBar.To = barTo;
        lifeforceBar.From = lerp2(barTo, barFrom, self.lifeforce/self.maxLifeforce);
        lifeforceBar.Color = lerpColor(Color3.new(0.3, 0, 0), Color3.new(1, 0, 0), self.lifeforce/self.maxLifeforce);

        local lifeforceBarOutline = visible.lifeforceBarOutline;
        lifeforceBarOutline.To = barTo + LIFEFORCE_BAR_OUTLINE_OFFSET;
        lifeforceBarOutline.From = barFrom - LIFEFORCE_BAR_OUTLINE_OFFSET;
        lifeforceBarOutline.Color = parseColor(self, options.lifeforceBarOutlineColor[1], true);
        lifeforceBarOutline.Transparency = options.lifeforceBarOutlineColor[2];
    end

    visible.lifeforceText.Visible = enabled and onScreen and options.lifeforceText;
    if visible.lifeforceText.Visible then
        local barFrom = corners.topRight + LIFEFORCE_BAR_OFFSET;
        local barTo = corners.bottomRight + LIFEFORCE_BAR_OFFSET;

        local lifeforceText = visible.lifeforceText;
        lifeforceText.Text = round(self.lifeforce) .. " blood";
        lifeforceText.Size = interface.sharedSettings.textSize;
        lifeforceText.Font = interface.sharedSettings.textFont;
        lifeforceText.Color = parseColor(self, options.lifeforceTextColor[1]);
        lifeforceText.Transparency = options.lifeforceTextColor[2];
        lifeforceText.Outline = options.lifeforceTextOutline;
        lifeforceText.OutlineColor = parseColor(self, options.lifeforceTextOutlineColor, true);
        lifeforceText.Position = lerp2(barTo, barFrom, self.lifeforce/self.maxLifeforce) - lifeforceText.TextBounds*0.5 - LIFEFORCE_TEXT_OFFSET;
    end
...
local EspInterface = {
	...
	teamSettings = {
		enemy = {
            ...
            lifeforceBar = false,
            lifeforceBarOutline = true,
            lifeforceBarOutlineColor = { Color3.new(), 0.5 },
            lifeforceText = false,
            lifeforceTextColor = { Color3.new(1,1,1), 1 },
            lifeforceTextOutline = true,
            lifeforceTextOutlineColor = Color3.new(),
            level = false,
...
getgenv().EspInterface = EspInterface;
...
function EspInterface.getLifeforce(player)
    local character = player and EspInterface.getCharacter(player);
    local lifeforce = character and character:FindFirstChild("Lifeforce");
    if lifeforce then
        return lifeforce.Value, lifeforce.MaxValue;
    end
    return 100, 100;
end
